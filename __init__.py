...
class GraphWrapper(object):
    ...
    def add(self, function, father_function):
        ...
        if function in self.nodes:
            ...
            node.lines = [self._build_function_text(function)]
            self.graph.append(node)
            self.nodes[function] = node

        father = self.nodes[father_function]
        father.add_outgoing_edge(BranchType.UnconditionalBranch, node)
        self.edges.add((father_function, function))

    def show(self):
        if Settings().get_bool("callgraph.showColorLeaves"):
            nodes_dst = set([edge[1] for edge in self.edges])
            nodes_src = set([edge[0] for edge in self.edges])

            leaves = nodes_dst - nodes_src
            for leave in leaves:
                self.nodes[leave].highlight = (
                    enums.HighlightStandardColor.RedHighlightColor
                )
        if Settings().get_bool("callgraph.showIndirectCalls"):
            for fun in self.nodes:
                if isinstance(fun, UndeterminedFunction):
                    self.nodes[fun].highlight = (
                        enums.HighlightStandardColor.BlueHighlightColor
                    )

        self.graph.show(
            "Callgraph of {} @ {:#x}".format(
                demangle_name(self.bv, self.root_function.name),
    ...
    while stack:
        ...
        for llil_block in func.llil:
            for llil in llil_block:
                if llil.operation.name in {"LLIL_CALL", "LLIL_TAILCALL"}:
                    if llil.dest.possible_values.type.name in {
                        "ImportedAddressValue",
                        "UndeterminedValue",
                    }:
                        if (
                            llil.dest.operation.name == "LLIL_LOAD"
                            and llil.dest.src.possible_values.type.name
                            ...
                            if dst_addr != 0:
                                dst_fun_addr_raw = bv.read(
                                    dst_addr, bv.arch.address_size
                                )
                                if len(dst_fun_addr_raw) == bv.arch.address_size:
                                    ...
                                    dst_fun_addr = int.from_bytes(
                                        ...
                                        (
                                            "little"
                                            if bv.arch.endianness.name == "LittleEndian"
                                            else "big"
                                        ),
                                    )
                            ...
                            llil.dest.possible_values.type.name == "UndeterminedValue"
                            and show_indirect
                        ):
                            # Indirect call
        ...
        for child_func in calls | indirect_calls | external_calls:
            ...
            if child_func not in visited:
                if not isinstance(child_func, UndeterminedFunction) and not isinstance(
                    child_func, ExternalFunction
                ):
...
class CallgraphThread(BackgroundTaskThread):
    def __init__(self, view, function, mode):
        super().__init__(f"Computing callgraph from {function.name} [{mode}]...")
        self.view = view
...
PluginCommand.register_for_function(
    "Callgraph\\Forward callgraph", "", _wrapper("normal")
)

PluginCommand.register_for_function(
    "Callgraph\\Reverse callgraph", "", _wrapper("reversed")
)
